...
<?php
$sql ="SELECT id from products WHERE ptype='4' ";
$query = $dbh -> prepare($sql);
$query->execute();
$results=$query->fetchAll(PDO::FETCH_OBJ);
...
<?php $sql = "SELECT products.oprice,products.title,type.typename,type.id,products.ribbon,products.price,products.id,products.description,products.Vimage1 from products join type on type.id=products.ptype WHERE type.id='4'";
$query = $dbh -> prepare($sql);
$query->execute();
$results=$query->fetchAll(PDO::FETCH_OBJ);
$cnt=1;
