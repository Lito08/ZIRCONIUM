...
<title>Zirconium - Electronics</title>
...
<div class="container">
	<h2 class="title-page">Electronics</h2>
	<nav>
...
<div class="card">
	...
	<article class="filter-group">
		...
		<?php
		$sql ="SELECT id,brand from products WHERE ptype='7' ";
...
<?php
$sql ="SELECT id from products WHERE ptype='7' ";
$query = $dbh -> prepare($sql);
$query->execute();
$results=$query->fetchAll(PDO::FETCH_OBJ);
...
<?php $sql = "SELECT products.oprice,products.title,type.typename,type.id,products.price,products.id,products.description,products.Vimage1 from products join type on type.id=products.ptype WHERE type.id='7'";
$query = $dbh -> prepare($sql);
$query->execute();
$results=$query->fetchAll(PDO::FETCH_OBJ);
$cnt=1;
