...
<div class="card">
	...
	<article class="filter-group">
		...
		<?php
		$sql ="SELECT id,brand from products WHERE ptype='6' ";
...
<?php $sql = "SELECT products.oprice,products.title,type.typename,type.id,products.price,products.id,products.description,products.Vimage1 from products join type on type.id=products.ptype WHERE type.id='6'";
$query = $dbh -> prepare($sql);
$query->execute();
$results=$query->fetchAll(PDO::FETCH_OBJ);
$cnt=1;
